// Copyright 2017-2019 Nathan Otterness
// Copyright 2020-2024 Joshua Bakita
// This file was originally part of the cuda_scheduling_examiner tool, but has
...
static __device__ inline uint64_t GlobalTimer64(void) {
  ...
  uint64_t ret;
  // The clock can roll over while we read the lo_bits in two cases:
  // 1. We read the hi_bits right as they changed, and lo_bits should be 0.
  // 2. We were preempted between reading hi_bits and hi_bits_2. In this case
  //    we need to re-read lo_bits, as they could be anything. Note that we
  //    assume that we'll never be suspended in an instruction-level preemption
  //    for more than ~4.29 seconds (which is how long it would take for the
  //    high bits to roll over again)---this allows us to just reread lo_bits.
  //    We also assume that this function will be preempted at at most one
  //    point during each invocation (the default timeslice length is 1-2 ms,
  //    so this assumption should always hold with the default scheduler).
